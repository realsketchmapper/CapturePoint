...
export class NMEAParser {
  ...
  /**
   * Converts NMEA coordinates (lat, lon) to maplibre format (lon, lat)
   * @param latitude Latitude in decimal degrees
   * @param longitude Longitude in decimal degrees
   * @returns Coordinates in [longitude, latitude] format for maplibre
   */
  static toMaplibreCoordinates(latitude: number, longitude: number): [number, number] {
    return [longitude, latitude];
  }

  /**
   * Converts NMEA coordinates (lat, lon) to MySQL Point format (lon, lat)
   * @param latitude Latitude in decimal degrees
   * @param longitude Longitude in decimal degrees
   * @returns Coordinates in {longitude, latitude} format for MySQL Point
   */
  static toMySQLPoint(latitude: number, longitude: number): { longitude: number, latitude: number } {
    return { longitude, latitude };
  }

  /**
   * Converts GGA data to maplibre coordinates
   * @param ggaData GGA data from NMEA
   * @returns Coordinates in [longitude, latitude] format for maplibre
   */
  static ggaToMaplibreCoordinates(ggaData: GGAData): [number, number] | null {
    if (ggaData.latitude === null || ggaData.longitude === null) {
      return null;
    }
    return this.toMaplibreCoordinates(ggaData.latitude, ggaData.longitude);
  }
  ...
  static ggaToMySQLPoint(ggaData: GGAData): { longitude: number, latitude: number } | null {
    ...
    return this.toMySQLPoint(ggaData.latitude, ggaData.longitude);
  }
}